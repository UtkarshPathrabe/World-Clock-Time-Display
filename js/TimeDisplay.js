...
function displayTimeCDT(){
	displayTime(moment().tz("America/Winnipeg"), 75, "#digital-time-cdt", "#analog-time-cdt");
}
function displayTimeEDT(){
	displayTime(moment().tz("America/New_York"), 75, "#digital-time-edt", "#analog-time-edt");
}
function displayTimeBST(){
	displayTime(moment().tz("Europe/London"), 75, "#digital-time-bst", "#analog-time-bst");
}
function displayTimeUTC(){
	displayTime(moment().utc(), 75, "#digital-time-utc", "#analog-time-utc");
}
function displayTimeIST(){
	displayTime(moment().tz("Asia/Kolkata"), 100, "#digital-time-ist", "#analog-time-ist");
}
function displayTimeSGP(){
	displayTime(moment().tz("Asia/Singapore"), 75, "#digital-time-sgp", "#analog-time-sgp");
}
function displayTimeHKT(){
	displayTime(moment().tz("Asia/Hong_Kong"), 75, "#digital-time-hkt", "#analog-time-hkt");
}
function displayTimeJST(){
	displayTime(moment().tz("Asia/Tokyo"), 75, "#digital-time-jst", "#analog-time-jst");
}
function displayTimeAEST(){
	displayTime(moment().tz("Australia/Sydney"), 75, "#digital-time-aest", "#analog-time-aest");
}
function displayTime(now, clockRadius, digitalClockDiv, analogClockCanvas){
	...
	var month = now.month();
	var year = now.year();
	var timeString = getDay(day) + " " + padZero(date) + "/" + padZero(month+1) + "/" + String(year) + " " + padZero(formatHour(hours)) + ":" + padZero(minutes) + ":" + padZero(seconds) + " " + getTimePeriod(hours);
	...
	function drawClockIndicators(){
		function drawClockIndicator(progress, color){
			...
			var startLength = 0.8;
			var endLength = 0.95;
			if(progress == 0/12 || progress == 3/12 || progress == 6/12 || progress == 9/12){
				thickness = 0.10;
				startLength = 0.77;
				endLength = 1;
			}
	...
	drawClockIndicators();
	drawArm((hours/12)+(minutes/(60*12)), 0.07, 0.50, "#000000");
	drawArm(minutes/60, 0.035, 0.75, "#000000");
	drawArm(seconds/60, 0.02, 1.00, "#FF0000");
}
